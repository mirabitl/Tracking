...
void binaryreader::processEvent(rbEvent* e)
{
  ...
  for (int id=0;id<MAXDIF;id++)
      ...
      for (int j=0;j<e->frameCount(id);j++)
	{
	  ...
	  if (e->bcid(idx)>_maxTime) _maxTime=e->bcid(idx);
	}
    }
  ...
  if (_maxTime<100) return;
  //if (rf) return;
  bool rf=false;
  std::bitset<16> plh;plh.reset();
  for (int id=0;id<MAXDIF;id++)
    if (e->frameCount(id))
      {
	...
	for (int j=0;j<e->frameCount(id);j++)
	  {
		...
		_timeMap.insert(p);
	      ...
	      {
		if (itm!=_timeMap.end()) itm->second.push_back(idx);
		if (itmm!=_timeMap.end()) itmm->second.push_back(idx);
		if (itmp!=_timeMap.end()) itmp->second.push_back(idx);
		if (itmmm!=_timeMap.end()) itmmm->second.push_back(idx);
		if (itmpp!=_timeMap.end()) itmpp->second.push_back(idx);
	      }
  ...
  if (rf)
    {
      ...
      if (trigger)
	for (int ip=1;ip<9;ip++)
	  if (plh[ip]==1) buildPosition(e,ip,6,false);
      ...
      for (auto x:_timeMap)
	{
	  ...
	  if (x.second.size()<nplanesmin*2) continue;
	  ...
	  for (int ip=0;ip<16;ip++)
	    {
	      if (u[ip]&&v[ip]) planes.set(ip);
	      if (u[ip]&&w[ip]) planes.set(ip);
	      if (w[ip]&&v[ip]) planes.set(ip);
	    }
	  uint32_t nplanes=planes.count();
	  if (nplanes<nplanesmin) continue;
	  ...
	  if (top_tk.size()<2) continue;
	  top_tk.regression();
	  top_tk.calculateChi2();	  

		  
	  if (top_tk.pchi2()<px2cut) continue;
	  if (abs(top_tk.dir().X())>thcut) continue;
	  if (abs(top_tk.dir().Y())>thcut) continue;
	  ...
	  //double ax,ay,bx,by;
	      ...
	      if (bot_tk.size()>=2 && bot_tk.pchi2()>px2cut)
		{
		  ...
		  if (zcross<z[5]+1 || zcross>z[4]-1) continue;
		  fflush(stdout);
		...
		if (stopmu ){
		  ...
		  if (pe.X()<-27.648+3) tkok=false;
		  if (pe.X()>27.648-3) tkok=false;
		  if (pe.X()<0)
		    {
		      if (pe.Y()<-1.*T30*pe.X()+3) tkok=false;
		      if (pe.Y()>T30*pe.X()+55.296-3) tkok=false;
		    }
		  else
		    {
		      if (pe.Y()<T30*pe.X()+3) tkok=false;
		      if (pe.Y()>-1.*T30*pe.X()+55.296-3) tkok=false;
		    }
		  if (tkok)
		    {
		      vstop[nstop]=top_tk;
		  ...
		  if (nstop%20==0 && nstop>0)
		    {
		      if (nstop>1000) continue;
		      if (nstop==lastprocessed) continue;
		      lastprocessed=nstop;
		      hcomb->Reset();
	  ...
	  for (int ip=1;ip<=8;ip++)
	      ...
	      for (auto x=bot_tk.points().begin();x!=bot_tk.points().end();x++)
		{
	      ...
	      tp.regression();
	      tp.calculateChi2();
	      if (tp.pchi2()<px2cut) continue;

	      if (ip==1 && !(tp.planUsed(2) && tp.planUsed(3))) continue;
	      if (ip==1 && !(tp.planUsed(2) && tp.planUsed(8))) continue;
	      
	      if (ip==8 && !(tp.planUsed(6) && tp.planUsed(7))) continue;
	      if (ip==8 && !(tp.planUsed(6) && tp.planUsed(1))) continue;
	      if (ip>1 &&ip<8 && !(tp.planUsed(ip-1) && tp.planUsed(ip+1))) continue;
	      if (ip>1 &&ip<8 && !(tp.planUsed(1) && tp.planUsed(8))) continue;
	      ROOT::Math::XYZPoint pe=tp.extrapolate(z[ip]);

	      if (pe.X()<-27.648+3) continue;
	      if (pe.X()>27.648-3) continue;
	      if (pe.X()<0)
		{
		  if (pe.Y()<-1.*T30*pe.X()+3) continue;
		  if (pe.Y()>T30*pe.X()+55.296-3) continue;
		}
	      else
		{
		  if (pe.Y()<T30*pe.X()+3) continue;
		  if (pe.Y()>-1.*T30*pe.X()+55.296-3) continue;
		}
	      std::stringstream splane;
...
#undef HTFIT
		    ...
		    */
		    ...
		    std::vector < std::pair<uint32_t,uint32_t> > hbinsy; hbinsy.clear();
  ...
  
{
  ...
  splane<<"/gric/PLANE"<<plane<<"/";
  std::bitset<128> ub;ub.reset();
  std::bitset<128> vb;vb.reset();
  std::bitset<128> wb;wb.reset();
  std::bitset<128> ub2;ub2.reset();
  std::bitset<128> vb2;vb2.reset();
  std::bitset<128> wb2;wb2.reset();
  std::bitset<128> ub3;ub3.reset();
  std::bitset<128> vb3;vb3.reset();
  std::bitset<128> wb3;wb3.reset();
  ...
  for (auto y:hits)
    {
      ...
      int sshift=0;
      if (dir==4 || dir==5 || dir==7) sshift=64;
      for (int k=0;k<64;k++)
	{
	  if (e->pad0(y,k)||e->pad1(y,k)) {
	    if (udir) ub.set(k+sshift);
	    if (vdir) vb.set(k+sshift);
	    if (wdir) wb.set(k+sshift);
	  }
	  if (e->pad0(y,k)&& !e->pad1(y,k)) {
	  if (udir) ub2.set(k+sshift);
	  if (vdir) vb2.set(k+sshift);
	  if (wdir) wb2.set(k+sshift);
	}
	if (e->pad0(y,k) && e->pad1(y,k)) {
	    if (udir) ub3.set(k+sshift);
	    if (vdir) vb3.set(k+sshift);
	    if (wdir) wb3.set(k+sshift);
	  }
  ...
  std::vector<float> vx_u;vx_u.clear();
  std::vector<float> vx_v;vx_v.clear();
  std::vector<float> vx_w;vx_w.clear();
  std::vector<float> sx_u;
  std::vector<float> sx_v;
  std::vector<float> sx_w;

  if (ub2.count()!=0) ub=ub2;
  if (vb2.count()!=0) vb=vb2;
  if (wb2.count()!=0) wb=wb2;

  if (ub3.count()!=0) ub=ub3;
  if (vb3.count()!=0) vb=vb3;
  if (wb3.count()!=0) wb=wb3;
  /*  */
  if (ub.count()>0)
    {
      int f=1000,l=-1;
	...
	{
	  if (ub[i]&&f>128) f=i;
	  if (i<64 && ub[i]) _bs[6*(plane-1)] |= (1<<i);
	  if (i>=64 && ub[i]) _bs[6*(plane-1)+1] |= (1<<(i-64));
	  if (ub[i]==0&&f<128 &&l<0)
	    {
	      l=i-1;
	      first.push_back(f);
  ...
  if (vb.count()>0)
	...
	{
	  if (vb[i]&&f>128) f=i;
	  if (i<64 && vb[i]) _bs[6*(plane-1)+2] |= (1<<i);
	  if (i>=64 && vb[i]) _bs[6*(plane-1)+3] |= (1<<(i-64));
  ...
  if (wb.count()>0)
	...
	{
	  if (wb[i]&&f>128) f=i;
	  if (i<64 && wb[i]) _bs[6*(plane-1)+4] |= (1<<i);
	  if (i>=64 && wb[i]) _bs[6*(plane-1)+5] |= (1<<(i-64));
  ...
  float X_g,Y_g;
  if (buvw)
    {X_g=(X_uv+X_uw+X_vw)/3.;Y_g=(Y_uv+Y_uw+Y_vw)/3.;
	  ...
	  for (auto xw:vx_w)
	    {
	      ...
	      a_w=T30;a_v=-1.*T30,b_w=xw*TSTEP;b_v=(128-xv)*TSTEP;
...
void binaryreader::buildPosition(rbEvent* e,uint32_t plane,uint32_t ddmax,bool all)
{
  ...
  for (auto x:_timeMap)
    {
      // First cut on time
      if (ddmax>0 && (_maxTime-x.first)>ddmax) continue;
      if (ddmax=0 && (_maxTime-x.first)<30) continue;
      if (x.second.size()<2) continue; // At least 2 direction
      std::bitset<128> ub;ub.reset();
      std::bitset<128> vb;vb.reset();
      std::bitset<128> wb;wb.reset();
      for (auto y:x.second)
	  ...
	  if (x_w>0)
	    {
	      buvw=true;
	      X_uvw=X;Y_uvw=Y;
	      huvw->Fill(X,Y);  if (!all) hst->Fill(21.);
